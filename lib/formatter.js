...
async function findPrettier (editor) {
  ...
  const detectInDirectory = (...dirs) =>
    detectFile(Path.join(...dirs, 'node_modules', 'prettier', 'index.js')) ||
    detectFile(Path.join(...dirs, 'node_modules', 'prettier', 'index.cjs'))

  if (shouldFind('inCurrentFileDirectory')) {
    detectInDirectory(Path.dirname(editor.getPath()))
  }

  const projectDir = atom.project.getDirectories().find(dir => dir.contains(editor.getPath()))?.getPath()

  if (!libPath && projectDir) {
    let cur = Path.dirname(Path.dirname(editor.getPath()))
    const root = Path.parse(cur).root
    while (cur && cur.length > projectDir.length && cur !== root) {
      detectInDirectory(cur)
      if (libPath) break
  ...
  if (!libPath && shouldFind('inCurrentProject')) {
    projectDir && detectInDirectory(projectDir)
  }
  ...
  const version = require(Path.join(Path.dirname(libPath), "package.json")).version

  if (version.split(".")[0] > 2) {
    const bin = Path.join(Path.dirname(libPath), "bin", "prettier.cjs")
    return {prettier: Fork(bin, cwd), which, v3: true}
  }

  const bin = Path.join(Path.dirname(libPath), "bin-prettier.js")
  return {prettier: Fork(bin, cwd), which, v3: true}
}
...
class Formatter {
  ...
  async format (onSave) {
    ...
    const {prettier, which, v3} = await findPrettier(editor)
    ...
    if (v3) {
      ...
      await prettier.formatEditor(editor, !!onSave)
        // .then(formatted => {
        //   if (formatted) return
        ...
        // })
        // .catch(err => {
        //   console.error("Failed to format with prettier v3", err)
        // })
